...
const { randomPop, sayOkay, sleep, hostingPath } = require('./util');
...
app.intent('Default Welcome Intent', async (conv) => {
    conv.ask(`<speak><voice gender="male"><prosody rate="90%" pitch="-2st">Pairing...</prosody></voice></speak>`);

    // CHANGEME this is making it wait at the start, not the middle
    await sleep(3000);
...
app.intent(['Default Fallback Intent', 'clapper_connector'], (conv) => {
    ...
    conv.close(
        new SimpleResponse({
            ...
            speech: `<speak>${okayPhrase}!<break time='500ms' /><audio src='${hostingPath('audio/clap-1.mp3')}' /></speak>`,
        }),
        new BasicCard({
